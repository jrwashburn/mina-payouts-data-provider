...
const requiresSSL = Boolean(process.env.SLDB_REQUIRE_SSL);
const sldb = createLedgerPool(requiresSSL);
...
export async function insertBatch(dataArray: any[], hash: string, nextEpoch: number | null) {
...
function buildLedgerEntries(resultRows: any[]){
